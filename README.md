...
Put `bin/` on your `PATH` (or run `pnpm for-agents ...` from this directory).

	for-agents check [--diff] [DIR...]
...
## What a managed AGENTS.md looks like
	...
	<!-- for-agents
	# This file is generated by `for-agents` (in ~/code/system/for-agents) ...
...
3. Run `for-agents update .` in it.

## Changing the template

Edit `templates/AGENTS.md.vto`, commit, then `for-agents check --diff` to see
what changes where, and `for-agents update` to commit it everywhere.
